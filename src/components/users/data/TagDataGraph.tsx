...
export const TagDataGraph = () => {
  ...
  //描画用データリスト。横軸時間のためにdateキーを追加している
  useEffect(() => {
    const _checkedDataList: TagData[] = [];
    dataList.forEach((data) => {
      var i = 0;
      //チェックされてたらそのtagIdにフラグをたて、描画用データリストにデータ入れてる。書き方他に思い浮かばんかった
      tagList.map((tag) => {
        if (tag.checked) {
  ...
  return (
    ...
    >
      ...
      <Legend />
      <ReferenceLine y={6.5} label="" stroke="red" />
      <ReferenceLine y={5.5} label="" stroke="red" />
      {/* <Line type="monotone" dataKey="k" stroke="#8884d8" dot={<CustomizedDot />} /> */}
      <Line type="monotone" dataKey="ph" stroke="#82ca9d" dot={<CustomizedDot />} />
    </LineChart>
  );
};

function CustomTooltip({ payload, label, active }: any) {
  const date = reformToDate(label);
  if (active) {
    return (
      <div className="custom-tooltip">
        <p className="label">{`${date.month}/${date.day} ${date.hour}:${date.minute}`}</p>
        {payload && <p>{`PH：${payload[0].value}`}</p>}
      </div>
    );
  }
